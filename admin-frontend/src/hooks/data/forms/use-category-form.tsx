...
const useCategoryForm = (formID = '') => {
    ...
    useEffect(() => {
        (loading && formID !== '') && handleGetQueryRequest(`${ROUTES.commonCategoryRoute}/${formID}`)
    }, [loading, formID])
    ...
    async function handleFormSubmission(data: Record<string, any>, handleConfirmation: () => void){
        const response = await HandlePostRequest({
            ...
            toastDescription: `Category has ${formID === '' ? 'created' : 'updated'} successfully!`,
        });
        if (response?.status === 200) {
            handleConfirmation()
        }
